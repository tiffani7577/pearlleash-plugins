...
class KawaiiShimmerVerbLookAndFeel : public juce::LookAndFeel_V4 {
public:
    struct SpriteAtlas {
        juce::Image sheet;
        int frameCount = 1;
...
class KawaiiShimmerVerbEditor : public juce::AudioProcessorEditor, private juce::Timer
{
public:
    explicit KawaiiShimmerVerbEditor (KawaiiShimmerVerbProcessor& p) : AudioProcessorEditor (p), proc (p)
    {
        loadAssets();
        roomSizeAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("roomSize"), roomSizeRelay, nullptr);
        decayAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("decay"), decayRelay, nullptr);
        shimmerAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("shimmer"), shimmerRelay, nullptr);
        dampingAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("damping"), dampingRelay, nullptr);
...
private:
    KawaiiShimmerVerbProcessor& proc;
    KawaiiShimmerVerbLookAndFeel lnf;
    KawaiiShimmerVerbLookAndFeel::SpriteAtlas bgAtlas;
    juce::WebSliderRelay roomSizeRelay { "roomSize" };
    juce::WebSliderRelay decayRelay { "decay" };
    juce::WebSliderRelay shimmerRelay { "shimmer" };
    ...
    std::unique_ptr<juce::WebSliderParameterAttachment> decayAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> shimmerAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> dampingAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> mixAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> gainAttach;
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (KawaiiShimmerVerbEditor)
};
