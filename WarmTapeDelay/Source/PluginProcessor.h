...
public:
          ...
          apvts (*this, nullptr, juce::Identifier ("PARAMS"), createLayout())
    {
        ...
        versionManager.registerParameter ("wowFlutter", 0.5f, 1);
    ...
    static juce::AudioProcessorValueTreeState::ParameterLayout createLayout()
    {
        ...
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "delayTime", 1 }, "Delay Time", juce::NormalisableRange<float> (1.0f, 1200.0f), 250.0f));
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "feedback", 1 }, "Feedback", juce::NormalisableRange<float> (0.0f, 0.98f), 0.5f));
        ...
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "wowFlutter", 1 }, "Wow/Flutter", juce::NormalisableRange<float> (0.0f, 1.0f), 0.5f));
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "drive", 1 }, "Drive", juce::NormalisableRange<float> (0.5f, 4.0f), 1.5f));
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "tone", 1 }, "Tone", juce::NormalisableRange<float> (500.0f, 18000.0f), 4000.0f));
...
wowFlutterSmoothed.reset(srF, 0.020f); wowFlutterSmoothed.setCurrentAndTargetValue(0.5f);
driveSmoothed.reset(srF, 0.020f); driveSmoothed.setCurrentAndTargetValue(1.5f);
toneSmoothed.reset(srF, 0.020f); toneSmoothed.setCurrentAndTargetValue(4000.0f);
...
rawFB = juce::jlimit(0.0f, 0.98f, rawFB);
feedbackSmoothed.setTargetValue(rawFB);
float rawWF = apvts.getRawParameterValue("wowFlutter")->load();
rawWF = juce::jlimit(0.0f, 1.0f, rawWF);
wowFlutterSmoothed.setTargetValue(rawWF);
...
rawTone = juce::jlimit(500.0f, 18000.0f, rawTone);
toneSmoothed.setTargetValue(rawTone);
const int bufSize = kMaxDelay;
auto* dL = block.getChannelPointer(0);
auto* dR = block.getNumChannels() > 1 ? block.getChannelPointer(1) : block.getChannelPointer(0);
const size_t nSamples = block.getNumSamples();
for (size_t i = 0; i < nSamples; ++i) {
  float delayMs = delaySmoothed.getNextValue();
  float delaySamples = juce::jlimit(1.0f, (float)(bufSize - 4), delayMs * 0.001f * srF);
  float wf = wowFlutterSmoothed.getNextValue();
  float drive = driveSmoothed.getNextValue();
  float fb = feedbackSmoothed.getNextValue();
  float toneCutoff = toneSmoothed.getNextValue();
  float wowDepth = wf * 8.0f;
  float flutterDepth = wf * 2.0f;
  float wowRate = 0.5f;
  ...
  noiseState = noiseState * 0.99f + ((float)rand() / (float)RAND_MAX * 2.0f - 1.0f) * 0.01f;
  float wowOff = wowDepth * std::sin(2.0f * 3.14159265f * wowPhase);
  float flutterOff = flutterDepth * std::sin(2.0f * 3.14159265f * flutterPhase) + noiseState * flutterDepth;
  float readOffset = delaySamples - wowOff - flutterOff;
  readOffset = juce::jlimit(1.0f, (float)(bufSize - 4), readOffset);
  float readF = (float)writePos - readOffset;
  while (readF < 0.0f) readF += (float)bufSize;
  while (readF >= (float)bufSize) readF -= (float)bufSize;
  int r0 = (int)readF;
  float frac = readF - (float)r0;
  int rm1 = (r0 - 1 + bufSize) % bufSize;
  int r1 = (r0 + 1) % bufSize;
  int r2 = (r0 + 2) % bufSize;
  float cL0 = bufL[rm1], cL1 = bufL[r0], cL2 = bufL[r1], cL3 = bufL[r2];
  float cR0 = bufR[rm1], cR1 = bufR[r0], cR2 = bufR[r1], cR3 = bufR[r2];
  float t = frac;
  ...
  float delOutL = w0*cL0 + w1*cL1 + w2*cL2 + w3*cL3;
  float delOutR = w0*cR0 + w1*cR1 + w2*cR2 + w3*cR3;
  float satL = delOutL * drive;
  float satR = delOutR * drive;
  float adaaL, adaaR;
  float absL = std::abs(satL);
  if (absL < 1e-6f) adaaL = satL * (1.0f - satL*satL / 3.0f);
  else adaaL = (std::log(std::cosh(satL)) - std::log(std::cosh(fbStateL))) / (satL - fbStateL + 1e-9f);
  float absR = std::abs(satR);
  if (absR < 1e-6f) adaaR = satR * (1.0f - satR*satR / 3.0f);
  else adaaR = (std::log(std::cosh(satR)) - std::log(std::cosh(fbStateR))) / (satR - fbStateR + 1e-9f);
  fbStateL = satL; fbStateR = satR;
  float compGain = 1.0f / (1.0f + 0.3f * (std::abs(adaaL) + std::abs(adaaR)) * 0.5f);
  adaaL *= compGain / juce::jmax(0.01f, drive);
  adaaR *= compGain / juce::jmax(0.01f, drive);
  float toneCoeff = std::exp(-2.0f * 3.14159265f * toneCutoff / srF);
  float oneMinusCoeff = 1.0f - toneCoeff;
  toneStateL = toneStateL + oneMinusCoeff * (adaaL - toneStateL);
  toneStateR = toneStateR + oneMinusCoeff * (adaaR - toneStateR);
  float inL = dL[i];
  float inR = dR[i];
  float newL = juce::jlimit(-1.0f, 1.0f, inL + fb * toneStateL);
  float newR = juce::jlimit(-1.0f, 1.0f, inR + fb * toneStateR);
  bufL[writePos] = newL;
  bufR[writePos] = newR;
  ...
  float wetR = delOutR;
  float decorr = 0.0f;
  if (block.getNumChannels() > 1) decorr = (wetL - wetR) * 0.03f;
  wetL += decorr; wetR -= decorr;
  float mixVal = mixSmoothed.getNextValue();
  float dryL = inL;
  float processedL = wetL;
  float dryR = inR;
  float processedR = wetR;
  float outL = dryL * (1.0f - mixVal) + processedL * mixVal;
  float outR = dryR * (1.0f - mixVal) + processedR * mixVal;
...
private:
    ...
    case 0:
        ...
        if (auto* param = apvts.getParameter ("wowFlutter")) param->setValueNotifyingHost (param->convertTo0to1 (0.5f));
    ...
    case 1:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.4431f));
        if (auto* param = apvts.getParameter ("mix")) param->setValueNotifyingHost (param->convertTo0to1 (0.5283f));
        if (auto* param = apvts.getParameter ("wowFlutter")) param->setValueNotifyingHost (param->convertTo0to1 (0.4981f));
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (1.55f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (5750.0f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (-0.0052f));
        break;
    case 2:
        if (auto* param = apvts.getParameter ("delayTime")) param->setValueNotifyingHost (param->convertTo0to1 (360.7f));
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.5627f));
        if (auto* param = apvts.getParameter ("mix")) param->setValueNotifyingHost (param->convertTo0to1 (0.4493f));
        if (auto* param = apvts.getParameter ("wowFlutter")) param->setValueNotifyingHost (param->convertTo0to1 (0.491f));
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (1.55f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (5750.0f));
    ...
    case 3:
        if (auto* param = apvts.getParameter ("delayTime")) param->setValueNotifyingHost (param->convertTo0to1 (360.7f));
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.5032f));
        if (auto* param = apvts.getParameter ("mix")) param->setValueNotifyingHost (param->convertTo0to1 (0.5945f));
        if (auto* param = apvts.getParameter ("wowFlutter")) param->setValueNotifyingHost (param->convertTo0to1 (0.5853f));
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (1.7105f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (5750.0f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (2.9964f));
        break;
    case 4:
        if (auto* param = apvts.getParameter ("delayTime")) param->setValueNotifyingHost (param->convertTo0to1 (360.7f));
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.4634f));
        if (auto* param = apvts.getParameter ("mix")) param->setValueNotifyingHost (param->convertTo0to1 (0.4653f));
        if (auto* param = apvts.getParameter ("wowFlutter")) param->setValueNotifyingHost (param->convertTo0to1 (0.5005f));
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (1.55f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (5750.0f));
    ...
    case 5:
        if (auto* param = apvts.getParameter ("delayTime")) param->setValueNotifyingHost (param->convertTo0to1 (360.7f));
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.564f));
        if (auto* param = apvts.getParameter ("mix")) param->setValueNotifyingHost (param->convertTo0to1 (0.5736f));
        if (auto* param = apvts.getParameter ("wowFlutter")) param->setValueNotifyingHost (param->convertTo0to1 (0.4523f));
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (1.6015f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (5750.0f));
    ...
    case 6:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.533f));
        ...
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (3.8388f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (13232.4976f));
    ...
    case 7:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.6653f));
        ...
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (0.924f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (11667.9663f));
    ...
    case 8:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.5214f));
        ...
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (2.9687f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (3576.9316f));
    ...
    case 9:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.704f));
        ...
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (1.6006f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (9350.2174f));
    ...
    case 10:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.7308f));
        ...
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (3.8184f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (13508.8085f));
    ...
    case 11:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.9242f));
        ...
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (3.8634f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (1305.3808f));
    ...
    case 12:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.0665f));
        ...
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (0.7383f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (760.2381f));
    ...
    case 13:
        ...
        if (auto* param = apvts.getParameter ("feedback")) param->setValueNotifyingHost (param->convertTo0to1 (0.9396f));
        ...
        if (auto* param = apvts.getParameter ("drive")) param->setValueNotifyingHost (param->convertTo0to1 (0.7543f));
        if (auto* param = apvts.getParameter ("tone")) param->setValueNotifyingHost (param->convertTo0to1 (989.0107f));
