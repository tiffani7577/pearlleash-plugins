...
class CelestialReverbProcessor : public juce::AudioProcessor, public juce::AudioProcessorValueTreeState::Listener
{
public:
    CelestialReverbProcessor()
          ...
          apvts (*this, nullptr, "PARAMETERS", createParameterLayout())
    {
        ...
        licensed = CelestialReverbLicense::checkLicense();
        if (! licensed)
            juce::Logger::writeToLog ("CelestialReverb: unlicensed copy — enter your serial to activate.");
        ...
        cloudSync.prepare ("com.pearlleash.celestialreverb",
                           juce::String(),
                           "1.0.0");
        versionManager.registerParameter ("reverbTime", 5.0f, 1);
    ...
    ~CelestialReverbProcessor() override
    {
        cloudSync.stopSync();
    ...
    static juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
    {
        ...
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "reverbTime", 1 }, "Reverb Time", juce::NormalisableRange<float> (0.1f, 20.0f, 0.0f, 1.0f), 5.0f));
    ...
    void prepareToPlay (double sampleRate, int samplesPerBlock) override
    {
        ...
        rawParam_reverbTime = apvts.getRawParameterValue ("reverbTime");
        rawParam_gain = apvts.getRawParameterValue ("gain");
        rawParam_bypass = apvts.getRawParameterValue ("bypass");
        sm_reverbTime.reset (rawParam_reverbTime != nullptr ? rawParam_reverbTime->load() : 5.0f, 25.0f, sampleRate);
    ...
    shimmerReverbDsp_.setParameters (
        0.55f,
        juce::jlimit (0.0f, 1.0f, (smoothedParam ("reverbTime") - 0.1f) / 19.9f),
        0.45f,
        0.50f);
    ...
    void reset() override
    {
        ...
        truePeakLeft.reset();
        truePeakRight.reset();
        if (rawParam_reverbTime != nullptr)
            sm_reverbTime.reset (rawParam_reverbTime->load(), 25.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
    ...
    void processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages) override
    {
        ...
        if (rawParam_reverbTime != nullptr)
            sm_reverbTime.setTarget (rawParam_reverbTime->load());
        sm_reverbTime.update (getSampleRate());
        if (rawParam_gain != nullptr)
    ...
    const juce::String getName() const noexcept override { return "CelestialReverb"; }
...
public:
    ...
    std::atomic<float>* rawParam_reverbTime = nullptr;
    std::atomic<float>* rawParam_gain = nullptr;
    std::atomic<float>* rawParam_bypass = nullptr;
    WoManusSmoothedParameter<float> sm_reverbTime;
    ...
    {
        if (strcmp (id, "reverbTime") == 0) return sm_reverbTime.getNextValue();
    ...
    {
        if (strcmp (id, "reverbTime") == 0) return rawParam_reverbTime != nullptr ? rawParam_reverbTime->load() : fallback;
...
private:
    ...
    void applyPreset (int index)
    {
        ...
        {
    case 0:
        if (auto* param = apvts.getParameter ("reverbTime")) param->setValueNotifyingHost (param->convertTo0to1 (5.0f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (0.0f));
        break;
    case 1:
        if (auto* param = apvts.getParameter ("reverbTime")) param->setValueNotifyingHost (param->convertTo0to1 (9.055f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (0.0f));
        break;
    case 2:
        if (auto* param = apvts.getParameter ("reverbTime")) param->setValueNotifyingHost (param->convertTo0to1 (9.055f));
        ...
        break;
    case 3:
        if (auto* param = apvts.getParameter ("reverbTime")) param->setValueNotifyingHost (param->convertTo0to1 (15.025f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (0.0f));
        break;
    case 4:
        if (auto* param = apvts.getParameter ("reverbTime")) param->setValueNotifyingHost (param->convertTo0to1 (9.055f));
        ...
        break;
    case 5:
        if (auto* param = apvts.getParameter ("reverbTime")) param->setValueNotifyingHost (param->convertTo0to1 (4.08f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (0.0f));
        break;
    case 6:
        if (auto* param = apvts.getParameter ("reverbTime")) param->setValueNotifyingHost (param->convertTo0to1 (4.08f));
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CelestialReverbProcessor)
};
