...
class NebulaReverbProcessor : public juce::AudioProcessor, public juce::AudioProcessorValueTreeState::Listener
{
public:
    NebulaReverbProcessor()
          ...
          apvts (*this, nullptr, "PARAMETERS", createParameterLayout())
    {
        ...
        licensed = NebulaReverbLicense::checkLicense();
        if (! licensed)
            juce::Logger::writeToLog ("NebulaReverb: unlicensed copy — enter your serial to activate.");
        ...
        cloudSync.prepare ("com.pearlleash.nebulareverb",
                           juce::String(),
                           "1.0.0");
        versionManager.registerParameter ("predelayTime", 20.0f, 1);
        versionManager.registerParameter ("gain", 0.0f, 1);
        versionManager.registerParameter ("bypass", 0.0f, 1);
        versionManager.registerParameter ("time", 250.0f, 1);
        versionManager.registerParameter ("feedback", 0.35f, 1);
        versionManager.registerParameter ("mix", 0.5f, 1);
        versionManager.registerParameter ("stereoOffset", 0.0f, 1);
        versionManager.registerParameter ("interpQuality", 2.0f, 1);
        versionManager.registerParameter ("dualTap", 0.0f, 1);
        versionManager.registerParameter ("tap2Offset", 12.0f, 1);
        versionManager.registerParameter ("tap2Mix", 0.35f, 1);
    ...
    ~NebulaReverbProcessor() override
    {
        cloudSync.stopSync();
    ...
    static juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
    {
        ...
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "predelayTime", 1 }, "Predelay", juce::NormalisableRange<float> (0.0f, 100.0f, 0.0f, 1.0f), 20.0f));
        ...
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "time", 1 }, "Time", juce::NormalisableRange<float> (1.0f, 1200.0f, 0.0f, 0.3f), 250.0f));
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "feedback", 1 }, "Feedback", juce::NormalisableRange<float> (0.0f, 0.95f, 0.0f, 0.75f), 0.35f));
        ...
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "stereoOffset", 1 }, "Stereo Offset", juce::NormalisableRange<float> (-25.0f, 25.0f, 0.0f, 1.0f), 0.0f));
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "interpQuality", 1 }, "Interp Quality", juce::NormalisableRange<float> (0.0f, 3.0f, 0.0f, 1.0f), 2.0f));
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "dualTap", 1 }, "Dual Tap", juce::NormalisableRange<float> (0.0f, 1.0f, 0.0f, 1.0f), 0.0f));
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "tap2Offset", 1 }, "Tap 2 Offset", juce::NormalisableRange<float> (1.0f, 80.0f, 0.0f, 1.0f), 12.0f));
        layout.add (std::make_unique<juce::AudioParameterFloat> (juce::ParameterID { "tap2Mix", 1 }, "Tap 2 Mix", juce::NormalisableRange<float> (0.0f, 1.0f, 0.0f, 1.0f), 0.35f));
    ...
    void prepareToPlay (double sampleRate, int samplesPerBlock) override
    {
        ...
        rawParam_predelayTime = apvts.getRawParameterValue ("predelayTime");
        rawParam_gain = apvts.getRawParameterValue ("gain");
        rawParam_bypass = apvts.getRawParameterValue ("bypass");
        rawParam_time = apvts.getRawParameterValue ("time");
        rawParam_feedback = apvts.getRawParameterValue ("feedback");
        rawParam_mix = apvts.getRawParameterValue ("mix");
        rawParam_stereoOffset = apvts.getRawParameterValue ("stereoOffset");
        rawParam_interpQuality = apvts.getRawParameterValue ("interpQuality");
        rawParam_dualTap = apvts.getRawParameterValue ("dualTap");
        rawParam_tap2Offset = apvts.getRawParameterValue ("tap2Offset");
        rawParam_tap2Mix = apvts.getRawParameterValue ("tap2Mix");
        rawParam_width = apvts.getRawParameterValue ("width");
        rawParam_decorrelate = apvts.getRawParameterValue ("decorrelate");
        sm_predelayTime.reset (rawParam_predelayTime != nullptr ? rawParam_predelayTime->load() : 20.0f, 25.0f, sampleRate);
        ...
        sm_time.reset (rawParam_time != nullptr ? rawParam_time->load() : 250.0f, 25.0f, sampleRate);
        sm_feedback.reset (rawParam_feedback != nullptr ? rawParam_feedback->load() : 0.35f, 25.0f, sampleRate);
        sm_mix.reset (rawParam_mix != nullptr ? rawParam_mix->load() : 0.5f, 30.0f, sampleRate);
        sm_stereoOffset.reset (rawParam_stereoOffset != nullptr ? rawParam_stereoOffset->load() : 0.0f, 25.0f, sampleRate);
        sm_interpQuality.reset (rawParam_interpQuality != nullptr ? rawParam_interpQuality->load() : 2.0f, 25.0f, sampleRate);
        sm_dualTap.reset (rawParam_dualTap != nullptr ? rawParam_dualTap->load() : 0.0f, 25.0f, sampleRate);
        sm_tap2Offset.reset (rawParam_tap2Offset != nullptr ? rawParam_tap2Offset->load() : 12.0f, 25.0f, sampleRate);
        sm_tap2Mix.reset (rawParam_tap2Mix != nullptr ? rawParam_tap2Mix->load() : 0.35f, 30.0f, sampleRate);
        ...
        stereoInterpolatedDelayDsp_.prepare (dspSampleRate, 1.25f);
    ...
    void reset() override
    {
        ...
        truePeakLeft.reset();
        truePeakRight.reset();
        if (rawParam_predelayTime != nullptr)
            sm_predelayTime.reset (rawParam_predelayTime->load(), 25.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        ...
        if (rawParam_time != nullptr)
            sm_time.reset (rawParam_time->load(), 25.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        if (rawParam_feedback != nullptr)
            sm_feedback.reset (rawParam_feedback->load(), 25.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        if (rawParam_mix != nullptr)
            sm_mix.reset (rawParam_mix->load(), 30.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        if (rawParam_stereoOffset != nullptr)
            sm_stereoOffset.reset (rawParam_stereoOffset->load(), 25.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        if (rawParam_interpQuality != nullptr)
            sm_interpQuality.reset (rawParam_interpQuality->load(), 25.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        if (rawParam_dualTap != nullptr)
            sm_dualTap.reset (rawParam_dualTap->load(), 25.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        if (rawParam_tap2Offset != nullptr)
            sm_tap2Offset.reset (rawParam_tap2Offset->load(), 25.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        if (rawParam_tap2Mix != nullptr)
            sm_tap2Mix.reset (rawParam_tap2Mix->load(), 30.0f, preparedSampleRate_ > 0.0 ? (float) preparedSampleRate_ : 48000.0f);
        ...
        stereoInterpolatedDelayDsp_.reset();
        ...
        // ---- built-in block: AllpassDiffuser (JUCE builtin) ----
    {
        ...
        // ---- built-in block: InterpolatedDelayLine (JUCE builtin) ----
    {
        const float timeMs = juce::jlimit (1.0f, 1200.0f, smoothedParam ("time"));
        const float feedback = juce::jlimit (0.0f, 0.95f, smoothedParam ("feedback"));
        const float mix = juce::jlimit (0.0f, 1.0f, smoothedParam ("mix"));
        const float stereoOffsetMs = StereoInterpolatedDelayPair::clampStereoOffsetMs (
            smoothedParam ("stereoOffset"));
        const int interpQ = juce::jlimit (0, 3, (int) std::lround ((double) smoothedParam ("interpQuality")));
        stereoInterpolatedDelayDsp_.setInterpolationQuality (
            static_cast<DelayInterpolationQuality> (interpQ));
        const bool dualTap = smoothedParam ("dualTap") >= 0.5f;
        const float tap2OffsetMs = juce::jlimit (1.0f, 80.0f, smoothedParam ("tap2Offset"));
        const float tap2Mix = juce::jlimit (0.0f, 1.0f, smoothedParam ("tap2Mix"));

        auto* left = block.getChannelPointer (0);
        float* right = block.getNumChannels() > 1 ? block.getChannelPointer (1) : left;
        for (size_t i = 0; i < block.getNumSamples(); ++i)
        {
            float delayMsFrame = timeMs;

            float l = left[i];
            float r = right[i];
            stereoInterpolatedDelayDsp_.processSample (
                l, r, delayMsFrame, stereoOffsetMs, feedback, mix,
                dualTap, tap2OffsetMs, tap2Mix);
            left[i] = l;
    ...
    void processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages) override
    {
        ...
        if (rawParam_predelayTime != nullptr)
            sm_predelayTime.setTarget (rawParam_predelayTime->load());
        sm_predelayTime.update (getSampleRate());
        if (rawParam_gain != nullptr)
        ...
        if (rawParam_time != nullptr)
            sm_time.setTarget (rawParam_time->load());
        sm_time.update (getSampleRate());
        if (rawParam_feedback != nullptr)
            sm_feedback.setTarget (rawParam_feedback->load());
        sm_feedback.update (getSampleRate());
        if (rawParam_mix != nullptr)
            sm_mix.setTarget (rawParam_mix->load());
        sm_mix.update (getSampleRate());
        if (rawParam_stereoOffset != nullptr)
            sm_stereoOffset.setTarget (rawParam_stereoOffset->load());
        sm_stereoOffset.update (getSampleRate());
        if (rawParam_interpQuality != nullptr)
            sm_interpQuality.setTarget (rawParam_interpQuality->load());
        sm_interpQuality.update (getSampleRate());
        if (rawParam_dualTap != nullptr)
            sm_dualTap.setTarget (rawParam_dualTap->load());
        sm_dualTap.update (getSampleRate());
        if (rawParam_tap2Offset != nullptr)
            sm_tap2Offset.setTarget (rawParam_tap2Offset->load());
        sm_tap2Offset.update (getSampleRate());
        if (rawParam_tap2Mix != nullptr)
            sm_tap2Mix.setTarget (rawParam_tap2Mix->load());
        sm_tap2Mix.update (getSampleRate());
        if (rawParam_width != nullptr)
    ...
    const juce::String getName() const noexcept override { return "NebulaReverb"; }
...
public:
    ...
    std::atomic<float>* rawParam_predelayTime = nullptr;
    std::atomic<float>* rawParam_gain = nullptr;
    std::atomic<float>* rawParam_bypass = nullptr;
    std::atomic<float>* rawParam_time = nullptr;
    std::atomic<float>* rawParam_feedback = nullptr;
    std::atomic<float>* rawParam_mix = nullptr;
    std::atomic<float>* rawParam_stereoOffset = nullptr;
    std::atomic<float>* rawParam_interpQuality = nullptr;
    std::atomic<float>* rawParam_dualTap = nullptr;
    std::atomic<float>* rawParam_tap2Offset = nullptr;
    std::atomic<float>* rawParam_tap2Mix = nullptr;
    std::atomic<float>* rawParam_width = nullptr;
    std::atomic<float>* rawParam_decorrelate = nullptr;
    WoManusSmoothedParameter<float> sm_predelayTime;
    WoManusSmoothedParameter<float> sm_gain;
    WoManusSmoothedParameter<float> sm_bypass;
    WoManusSmoothedParameter<float> sm_time;
    WoManusSmoothedParameter<float> sm_feedback;
    WoManusSmoothedParameter<float> sm_mix;
    WoManusSmoothedParameter<float> sm_stereoOffset;
    WoManusSmoothedParameter<float> sm_interpQuality;
    WoManusSmoothedParameter<float> sm_dualTap;
    WoManusSmoothedParameter<float> sm_tap2Offset;
    WoManusSmoothedParameter<float> sm_tap2Mix;
    ...
    {
        if (strcmp (id, "predelayTime") == 0) return sm_predelayTime.getNextValue();
        if (strcmp (id, "gain") == 0) return sm_gain.getNextValue();
        if (strcmp (id, "bypass") == 0) return sm_bypass.getNextValue();
        if (strcmp (id, "time") == 0) return sm_time.getNextValue();
        if (strcmp (id, "feedback") == 0) return sm_feedback.getNextValue();
        if (strcmp (id, "mix") == 0) return sm_mix.getNextValue();
        if (strcmp (id, "stereoOffset") == 0) return sm_stereoOffset.getNextValue();
        if (strcmp (id, "interpQuality") == 0) return sm_interpQuality.getNextValue();
        if (strcmp (id, "dualTap") == 0) return sm_dualTap.getNextValue();
        if (strcmp (id, "tap2Offset") == 0) return sm_tap2Offset.getNextValue();
        if (strcmp (id, "tap2Mix") == 0) return sm_tap2Mix.getNextValue();
    ...
    {
        if (strcmp (id, "predelayTime") == 0) return rawParam_predelayTime != nullptr ? rawParam_predelayTime->load() : fallback;
        ...
        if (strcmp (id, "time") == 0) return rawParam_time != nullptr ? rawParam_time->load() : fallback;
        if (strcmp (id, "feedback") == 0) return rawParam_feedback != nullptr ? rawParam_feedback->load() : fallback;
        if (strcmp (id, "mix") == 0) return rawParam_mix != nullptr ? rawParam_mix->load() : fallback;
        if (strcmp (id, "stereoOffset") == 0) return rawParam_stereoOffset != nullptr ? rawParam_stereoOffset->load() : fallback;
        if (strcmp (id, "interpQuality") == 0) return rawParam_interpQuality != nullptr ? rawParam_interpQuality->load() : fallback;
        if (strcmp (id, "dualTap") == 0) return rawParam_dualTap != nullptr ? rawParam_dualTap->load() : fallback;
        if (strcmp (id, "tap2Offset") == 0) return rawParam_tap2Offset != nullptr ? rawParam_tap2Offset->load() : fallback;
        if (strcmp (id, "tap2Mix") == 0) return rawParam_tap2Mix != nullptr ? rawParam_tap2Mix->load() : fallback;
...
private:
    ...
    void applyPreset (int index)
    {
        ...
        {
    case 0:
        if (auto* param = apvts.getParameter ("predelayTime")) param->setValueNotifyingHost (param->convertTo0to1 (20.0f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (0.0f));
        break;
    case 1:
        if (auto* param = apvts.getParameter ("predelayTime")) param->setValueNotifyingHost (param->convertTo0to1 (45.0f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (0.0f));
        break;
    case 2:
        if (auto* param = apvts.getParameter ("predelayTime")) param->setValueNotifyingHost (param->convertTo0to1 (45.0f));
        ...
        break;
    case 3:
        if (auto* param = apvts.getParameter ("predelayTime")) param->setValueNotifyingHost (param->convertTo0to1 (75.0f));
        if (auto* param = apvts.getParameter ("gain")) param->setValueNotifyingHost (param->convertTo0to1 (0.0f));
        break;
    case 4:
        if (auto* param = apvts.getParameter ("predelayTime")) param->setValueNotifyingHost (param->convertTo0to1 (45.0f));
    ...
    StereoInterpolatedDelayPair stereoInterpolatedDelayDsp_;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NebulaReverbProcessor)
};
