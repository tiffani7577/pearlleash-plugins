...
START_TS=$(date +%s)
echo "═══════════════════════════════════════════"
echo "   PlugForge — Building NebulaReverb"
echo "═══════════════════════════════════════════"
echo ""
...
echo "  ✅ Project ready."
echo ""
echo "Step 3 of 5 · Compiling NebulaReverb ($CONFIG — incremental when possible)..."
cmake --build build --config $CONFIG --target NebulaReverb_AU NebulaReverb_VST3 >> .build_log.txt 2>&1 || { echo "  ❌ Compile failed. Send .build_log.txt to Claude."; read -p "Press Enter to close."; exit 1; }
cmake --build build --config $CONFIG --target NebulaReverb_Standalone >> .build_log.txt 2>&1 || true
cmake --build build --config $CONFIG >> .build_log.txt 2>&1 || true
echo "  ✅ Compiled."
echo ""
echo "Step 4 of 5 · Installing AU / VST3 / CLAP + refreshing Logic AU cache..."
export PLUGIN_NAME="NebulaReverb"
export BUILD_DIR="$(pwd)/build"
export CONFIG
export AU_SUBTYPE="GMeJ"
export AU_MFR="Prll"
export AU_TYPE="aufx"
...
if [ "$FAST" = "1" ]; then
  ...
  if auval -v aufx GMeJ Prll >> .build_log.txt 2>&1; then
...
open "build/NebulaReverb_artefacts/$CONFIG/Standalone/NebulaReverb.app" 2>/dev/null || open "build/NebulaReverb_artefacts/Release/Standalone/NebulaReverb.app" 2>/dev/null || true
echo "  • Standalone opened for instant hear."
echo "  • In Logic: remove the plug-in from the slot and re-insert (loads new binary)."
echo "  • Hot path: PLUGFORGE_FAST=1 PLUGFORGE_BUILD_CONFIG=Debug ./DevIterate\ NebulaReverb.command"
echo ""
echo "Done."
