...
class BuiltinAwareBlendLookAndFeel : public juce::LookAndFeel_V4 {
public:
    struct SpriteAtlas {
        juce::Image sheet;
        int frameCount = 1;
...
class BuiltinAwareBlendEditor : public juce::AudioProcessorEditor, private juce::Timer
{
public:
    explicit BuiltinAwareBlendEditor (BuiltinAwareBlendProcessor& p) : AudioProcessorEditor (p), proc (p)
    {
        loadAssets();
        mixAmountAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("mixAmount"), mixAmountRelay, nullptr);
...
private:
    BuiltinAwareBlendProcessor& proc;
    BuiltinAwareBlendLookAndFeel lnf;
    BuiltinAwareBlendLookAndFeel::SpriteAtlas bgAtlas;
    juce::WebSliderRelay mixAmountRelay { "mixAmount" };
    ...
    juce::WebBrowserComponent webView {
        juce::WebBrowserComponent::Options{}
            ...
            .withOptionsFrom (mixAmountRelay)
            .withOptionsFrom (gainRelay)
            .withResourceProvider ([] (const auto& url) { return getResource (url); })
    };

    std::unique_ptr<juce::WebSliderParameterAttachment> mixAmountAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> gainAttach;
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BuiltinAwareBlendEditor)
};
