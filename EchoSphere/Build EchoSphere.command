...
START_TS=$(date +%s)
echo "═══════════════════════════════════════════"
echo "   PlugForge — Building EchoSphere"
echo "═══════════════════════════════════════════"
echo ""
...
echo "  ✅ Project ready."
echo ""
echo "Step 3 of 5 · Compiling EchoSphere ($CONFIG — incremental when possible)..."
cmake --build build --config $CONFIG --target EchoSphere_AU EchoSphere_VST3 >> .build_log.txt 2>&1 || { echo "  ❌ Compile failed. Send .build_log.txt to Claude."; read -p "Press Enter to close."; exit 1; }
cmake --build build --config $CONFIG --target EchoSphere_Standalone >> .build_log.txt 2>&1 || true
cmake --build build --config $CONFIG >> .build_log.txt 2>&1 || true
echo "  ✅ Compiled."
echo ""
echo "Step 4 of 5 · Installing AU / VST3 / CLAP + refreshing Logic AU cache..."
export PLUGIN_NAME="EchoSphere"
export BUILD_DIR="$(pwd)/build"
export CONFIG
export AU_SUBTYPE="QmuW"
export AU_MFR="Prll"
export AU_TYPE="aufx"
...
if [ "$FAST" = "1" ]; then
  ...
  if auval -v aufx QmuW Prll >> .build_log.txt 2>&1; then
...
open "build/EchoSphere_artefacts/$CONFIG/Standalone/EchoSphere.app" 2>/dev/null || open "build/EchoSphere_artefacts/Release/Standalone/EchoSphere.app" 2>/dev/null || true
echo "  • Standalone opened for instant hear."
echo "  • In Logic: remove the plug-in from the slot and re-insert (loads new binary)."
echo "  • Hot path: PLUGFORGE_FAST=1 PLUGFORGE_BUILD_CONFIG=Debug ./DevIterate\ EchoSphere.command"
echo ""
echo "Done."
