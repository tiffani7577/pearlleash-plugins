...
class StutterGateLookAndFeel : public juce::LookAndFeel_V4 {
public:
    struct SpriteAtlas {
        juce::Image sheet;
        int frameCount = 1;
...
class StutterGateEditor : public juce::AudioProcessorEditor, private juce::Timer
{
public:
    explicit StutterGateEditor (StutterGateProcessor& p) : AudioProcessorEditor (p), proc (p)
    {
        loadAssets();
        activePatternAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("activePattern"), activePatternRelay, nullptr);
        subdivisionAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("subdivision"), subdivisionRelay, nullptr);
        attackAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("attack"), attackRelay, nullptr);
        releaseAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("release"), releaseRelay, nullptr);
...
private:
    StutterGateProcessor& proc;
    StutterGateLookAndFeel lnf;
    StutterGateLookAndFeel::SpriteAtlas bgAtlas;
    juce::WebSliderRelay activePatternRelay { "activePattern" };
    juce::WebSliderRelay subdivisionRelay { "subdivision" };
    ...
    juce::WebBrowserComponent webView {
        juce::WebBrowserComponent::Options{}
            ...
            .withOptionsFrom (activePatternRelay)
            .withOptionsFrom (subdivisionRelay)
    ...
    std::unique_ptr<juce::WebSliderParameterAttachment> activePatternAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> subdivisionAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> attackAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> releaseAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> gainAttach;
    std::unique_ptr<juce::WebSliderParameterAttachment> mixAttach;
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (StutterGateEditor)
};
