...
#include "FeedbackNonlinearIsland.h"
...
public:
    void prepare (double newSampleRate)
    {
        ...
        constexpr float fdnTimeScale = 2.5f;
        ...
        {
            fdn[lane].prepare ((int) std::ceil (sampleRate * 0.24) + 8);
            fdnDelaySamples[lane] =
                (float) sampleRate * fdnTimes[lane] * fdnTimeScale;
        ...
        for (auto& island : laneFeedbackIsland_)
            island.prepare (sampleRate);
    ...
    void reset() noexcept
    {
        ...
        for (auto& island : laneFeedbackIsland_)
            island.reset();
        if (fbIsland_ != nullptr)
            fbIsland_->reset();
    }

    /** Optional Phase A3 tank soft-sat / damp island (nullptr = legacy path). */
    void setFeedbackNonlinearIsland (StereoFeedbackNonlinearIsland* island) noexcept
    {
        fbIsland_ = island;
    }

    void setFeedbackNonlinearParams (
        float drive,
        float damp,
        bool satEnable,
        int osMode) noexcept
    {
        fbDrive_ = FeedbackNonlinearIsland::clampDrive (drive);
        fbDamp_ = FeedbackNonlinearIsland::clampDamp (damp);
        fbSatEnable_ = satEnable;
        fbOsMode_ = osMode;
    }
    ...
    void processSample (float& left, float& right) noexcept
    {
        ...
        const float wetLeftRaw =
            (fdnReadScratch[0] + fdnReadScratch[2]
             + fdnReadScratch[4] + fdnReadScratch[6]
             - 0.35f * (fdnReadScratch[1] + fdnReadScratch[5]));
        const float wetRightRaw =
            (fdnReadScratch[1] + fdnReadScratch[3]
             + fdnReadScratch[5] + fdnReadScratch[7]
             - 0.35f * (fdnReadScratch[2] + fdnReadScratch[6]));
        const float wetLeft = wetLeftRaw * 0.5f;
        const float wetRight = wetRightRaw * 0.5f;

        // Keep output make-up gain out of the feedback tap so it is not
        // multiplied again by the dedicated shimmer-feedback gain below.
        const float octaveLeft = pitchShifter[0].process (wetLeftRaw * 0.34f);
        const float octaveRight = pitchShifter[1].process (wetRightRaw * 0.34f);
        ...
        for (std::size_t lane = 0; lane < fdn.size(); ++lane)
        {
            ...
            const float excitation =
                ...
                + octave * smoothedShimmer * 0.34f * 1.8f * sign;
            float regen = dampingState[lane] * feedback;
            if (fbIsland_ != nullptr)
            {
                // Each FDN lane MUST have its own feedback-island state. The previous
                // code shared the two StereoFeedbackNonlinearIsland instances
                // (left_/right_) across four lanes each, so a single one-pole damp +
                // ADAA state was advanced 4x per sample with unrelated lane signals.
                // That cross-contaminated the lanes (collapsing the reverb tail) and
                // corrupted the ADAA antiderivative memory (high-frequency squeak).
                // Per-lane islands keep each feedback path independent.
                regen = laneFeedbackIsland_[lane].process (
                    dampingState[lane], feedback, fbDrive_, fbDamp_, fbSatEnable_,
                    FeedbackNonlinearIsland::clampOsMode (fbOsMode_));
            }
            fdn[lane].write (sanitize (excitation + regen));
        }

        // Constant-power blend with exact dry/wet endpoints.
        const float dryGain = std::cos (smoothedMix * halfPi);
        const float wetGain = std::sin (smoothedMix * halfPi);
...
private:
    ...
    StereoFeedbackNonlinearIsland* fbIsland_ = nullptr;
    // One independent feedback island per FDN lane (state must not be shared).
    std::array<FeedbackNonlinearIsland, 8> laneFeedbackIsland_ {};
    float fbDrive_ = FeedbackNonlinearIsland::kDefaultDrive;
    float fbDamp_ = FeedbackNonlinearIsland::kDefaultDamp;
    bool fbSatEnable_ = true;
    int fbOsMode_ = 0;
};
