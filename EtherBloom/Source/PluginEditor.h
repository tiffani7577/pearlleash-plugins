...
class EtherBloomLookAndFeel : public juce::LookAndFeel_V4 {
public:
    struct SpriteAtlas {
        juce::Image sheet;
        int frameCount = 1;
...
class EtherBloomEditor : public juce::AudioProcessorEditor, private juce::Timer
{
public:
    explicit EtherBloomEditor (EtherBloomProcessor& p) : AudioProcessorEditor (p), proc (p)
    {
        loadAssets();
...
private:
    EtherBloomProcessor& proc;
    EtherBloomLookAndFeel lnf;
    EtherBloomLookAndFeel::SpriteAtlas bgAtlas;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (EtherBloomEditor)
};
