...
class HarmonicEQLookAndFeel : public juce::LookAndFeel_V4 {
public:
    struct SpriteAtlas {
        juce::Image sheet;
        int frameCount = 1;
...
class HarmonicEQEditor : public juce::AudioProcessorEditor, private juce::Timer
{
public:
    explicit HarmonicEQEditor (HarmonicEQProcessor& p) : AudioProcessorEditor (p), proc (p)
    {
        loadAssets();
        rootAttach = std::make_unique<juce::WebSliderParameterAttachment> (*proc.apvts.getParameter ("root"), rootRelay, nullptr);
...
private:
    HarmonicEQProcessor& proc;
    HarmonicEQLookAndFeel lnf;
    SpriteAtlas bgAtlas;
    juce::WebSliderRelay rootRelay { "root" };
    ...
    std::unique_ptr<juce::WebSliderParameterAttachment> rootAttach;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (HarmonicEQEditor)
};
